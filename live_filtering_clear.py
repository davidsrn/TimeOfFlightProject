...
def evaluate(weight_file_path, data_dir, output_dir, prob_thresh=0.5, nms_thresh=0.1, lw=3, display=False):
  ...
  Returns:
    None.
  """
  # Aqui creo que  basicamente le esta diciendo que es lo que espera por default
  ...
  with tf.Session() as sess:
    ...
    # A partir de aquí, empieza a tratar individualmente el input
...
class MainWindow():
    ...
    def processDepthFrame(self, depthCamera, frame, type):
        ...
        if self.drop < drop_imgs:
            ...
            self.background = self.background+cv2.GaussianBlur((np.array(depthFrame.depth).reshape((depthFrame.size.height, depthFrame.size.width))),(5,5),0)
            ...
            dep_res = (self.background - cv2.GaussianBlur(depth_matrix, (5,5), 0))
            cv2.imwrite("phase_bkg_substraction.jpg", ((f(dep_res))*255/(dep_res.max())).astype(np.uint8))

            dep_res = (cv2.morphologyEx((cv2.morphologyEx(self.background - depth_matrix, cv2.MORPH_OPEN, self.kernel)), cv2.MORPH_CLOSE, self.kernel))
            ...
            with tf.Graph().as_default():
                faces = evaluate("./out2", data_dir=amp_res, output_dir="./test", prob_thresh=0.5, nms_thresh=0.1, lw=3, display=False)
            # faces = np.array([])
            ...
            # thresh =
            thresh = cv2.dilate(cv2.erode(thresh, self.elip_kernel,  iterations = 1), self.elip_kernel,  iterations = 1)
            zeros = thresh == 0
            ...
            if persons.size != 0:
                for rect in persons:
                    ...
                    for face in faces:
                        # print("Face:")
                        # print(face)
                        if (rect[0] < face[0]) and (rect[1] < face[1]) and ((rect[0] + rect[2]) > face[2]) and ((rect[1] + rect[3]) > face[3]):
                            person = person + 1
                            break

            if faces.size != 0:
                for data in faces:
                    cv2.rectangle(dep_res, (int(data[0]), int(data[1])), (int(data[2]), int(data[3])), (0, 0, 0), 2)
                    # cv2.rectangle(amp_res, (_r[0], _r[1]), (_r[2], _r[3]), rect_color, _lw)

            if persons.size != 0:
                for data in persons:
                    cv2.rectangle(dep_res, (int(data[0]), int(data[1])), (int(data[0]+data[2]), int(data[1]+data[3])), (255, 0, 0), 2)
                    # print(data)
            ...
            cv2.putText(dep_res, ("Persons: " + str(person)),(0, 180), font, 1,(255,255,255),2,cv2.LINE_AA)

            cv2.imwrite("phase_with_detection.jpg", dep_res)
