...
class MainWindow():
    ...
    def processDepthFrame(self, depthCamera, frame, type):
        ...
        if self.drop < drop_imgs:
            ...
            # print("MIN")
            # print(frame.min())
            test_array = (np.array(depthFrame.depth).reshape((depthFrame.size.height, depthFrame.size.width)))

            test_array[test_array>5] = 5
            self.background = self.background+cv2.GaussianBlur(test_array,(7,7),0)
            ...
            test_array[test_array>5] = 5
            cv2.imwrite("test.jpg", ((f(test_array))*255/(test_array.max())).astype(np.uint8))
            ...
            dep_res = depth_matrix = test_array
            cv2.imwrite("phase_raw.jpg", ((f(dep_res))*255/(dep_res.max())).astype(np.uint8))

            dep_res = (self.background - cv2.GaussianBlur(test_array, (7,7), 0))
            ...
            _, conn, rects, centroids = cv2.connectedComponentsWithStats(image=((f(dep_res))*255/(dep_res.max())).astype(np.uint8), connectivity=8, ltype=cv2.CV_16U)
            # print(a)
            ...
            if persons.size != 0:
                for rect in persons:
                    ...
                    for face in faces:
                        # print("Face:")0
                        # print(face)
                        if (rect[0]-10 < face[0]) and (rect[1]-10 < face[1]) and ((rect[0] + rect[2]+10) > face[2]) and ((rect[1] + rect[3] + 10) > face[3]):
                            person = person + 1
                            break

            dep_res_rgb = cv2.cvtColor(((f(dep_res))*255/(dep_res.max())).astype(np.uint8), cv2.COLOR_GRAY2RGB)
            if faces.size != 0:
                for data in faces:
                    # print(data[4])
