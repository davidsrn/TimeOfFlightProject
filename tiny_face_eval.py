...
def getframe(raw_img):
    # In the next 4 lines, it create a third dimension to make it compatible
    frame = []
    print(raw_img.shape)
    for i in range(3):
        frame.append(raw_img)
    # The frame ready to  be enter the model.
    frame = np.stack(frame, axis=2)
    return frame
...
def evaluate(weight_file_path, data_dir, output_dir, prob_thresh=0.5, nms_thresh=0.1, lw=3, display=False):
  ...
  model = tiny_face_model.Model("out2")
  ...
  with tf.Session() as sess:
    ...
    for i in range(raw_frame.shape[0]):
        #print("QWERTY: ", raw_frame.shape)
        # Gets a single frame and normalize it to slace 0-255 dinamically
        raw_img = raw_frame[i]*255/raw_frame[i].max()

        frame = getframe(raw_img)
        # print("shape: ",raw_frame.shape)
        # print("FOR value: ",raw_frame.shape[0])
        ...
        for s in scales:
            ...
            img = cv2.resize(frame, (0, 0), fx=s, fy=s, interpolation=cv2.INTER_LINEAR)
        ...
        print("In frame #{} found {} faces in {:.2f} secs".format(i,refined_bboxes.shape[0], time.time() - start))
        # print("Found Faces: ",refined_bboxes.shape[0])
        new_name_img = str(i)+"coords_"+fname
        np.save(new_name_img, refined_bboxes)
        ...
        #tmp_route = os.path.splitext('/home/user/somefile.txt')[0]+'.jpg'
