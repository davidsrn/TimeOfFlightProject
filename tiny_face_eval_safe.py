...
def evaluate(weight_file_path, data_dir, output_dir, prob_thresh=0.5, nms_thresh=0.1, lw=3, display=False):
  ...
  x = tf.placeholder(tf.float32, [1, None, None, 3]) # number, heeight, width, color
  ...
  # Lo que hace es crear un arreglo vacio que luego llenara con un form
  ...
  with tf.Session() as sess:
    ...
    for filename in filenames:
      ...
      raw_img = cv2.imread(filename) #//matriz sea igual a lo que  hay en // hacer print a la img de en rawimg y hacer que los np array sean igual a ese tipo de dato e ignirar todo hasta este puntop
      # grayImage = cv2.cvtColor(raw_img, cv2.COLOR_BGR2GRAY)
      # print(grayImage.shape)
      # (thresh, blackAndWhiteImage) = cv2.threshold(grayImage, 0, 127, cv2.THRESH_BINARY)
...
def main():
  ...
  # 1. lee los parametros que recibira
  ...
  # check arguments | se comprueba que al menos tengan 3 argumentos de pesos, input y output.
  ...
  with tf.Graph().as_default():
    # Aqui manda a llamar a evaluate con los  valores que le pasamos en los argumentos
    evaluate(
